...
public class StreamEx3 {
  ...
  public static void main(String[] args) {
    ...
    // 스트림 문자열을 대문자로 변경한 후 리스트로 돌려받기 - collect
    List<String> list = stream2
      .map(String::toUpperCase)
      .collect(Collectors.toList());
    System.out.println(list);

    // 짝수의 개수 출력 - count (최종연산)
    ...
    long sum = stream4.filter(num -> num % 2 == 0).sum();
     ...
     * 최종 연산의 결과를 Optional 객체에 담아서 반환 ==> null 체크하기 위한 if 문 사용 안해도됨
     * 리턴 값이 널인지 아닌지 판단하는 코드 사용 ==> NullPointerException 발생을 막기 위해서
     */
